...
static int tfa98xx_spk_rotation; /* 0 = normal, 1 = rotated 180° */
...
/*
 * Apply speaker rotation by remapping TDM sink slot assignments.
 *
 * Android outputs 2ch stereo: Slot 0 = Left, Slot 1 = Right.
 * Two amps share each slot for hardware-level stereo duplication.
 *
 * Physical layout: dev0=UR(0x34), dev1=UL(0x35), dev2=LR(0x36), dev3=LL(0x37)
 *
 * Rotation states (slot_map[rot][dev] = TDM slot 0=L, 1=R):
 *   0° (landscape):    UL&LL→L, UR&LR→R  = {1, 0, 1, 0}
 *   90° (portrait):    UR&UL→L, LR&LL→R  = {0, 0, 1, 1}
 *   180° (landscape):  UR&LR→L, UL&LL→R  = {0, 1, 0, 1}
 *   270° (portrait):   LR&LL→L, UR&UL→R  = {1, 1, 0, 0}
 *
 * Registers written per device:
 *   0x13: TDMSI1SL [10:7] = main audio sink slot
 *         TDMSI2SL [14:11] = cross-channel reference for DSP protection
 */
static void tfa98xx_apply_spk_rotation(void)
{
	static const int slot_map[4][4] = {
		{1, 0, 1, 0}, /* 0°:   UL&LL→L(0), UR&LR→R(1) */
		{0, 0, 1, 1}, /* 90°:  UR&UL→L(0), LR&LL→R(1) */
		{0, 1, 0, 1}, /* 180°: UR&LR→L(0), UL&LL→R(1) */
		{1, 1, 0, 0}, /* 270°: LR&LL→L(0), UR&UL→R(1) */
	};
	int rot, i, slot;

	rot = tfa98xx_spk_rotation & 3;

	for (i = 0; i < tfa98xx_registered_handles; i++) {
		struct tfa98xx *tfa = tfa98xx_devices[i];
		...
		/* TDMSI1SL [10:7] = main audio input slot
		 * TDMSI2SL [14:11] = other channel for DSP protection ref */
		regmap_update_bits(tfa->regmap, 0x13,
			(0xF << 7) | (0xF << 11),
			(slot << 7) | ((slot ^ 1) << 11));

		pr_info("%s: dev %d (0x%02x): slot=%d rot=%d\n",
			__func__, i, tfa->i2c->addr, slot, rot);
	}
}
...
tfa98xx_tfa_start(struct tfa98xx *tfa98xx, int next_profile, int *vstep)
{
	...
	/* Override container TDM slot assignments for stereo duplication */
	if (err == tfa_error_ok)
		tfa98xx_apply_spk_rotation();
...
static int tfa98xx_info_spk_rotation(struct snd_kcontrol *kcontrol,
				     struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = 1;
...
static int tfa98xx_get_spk_rotation(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = tfa98xx_spk_rotation;
	return 0;
}

static int tfa98xx_set_spk_rotation(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	int rotation = ucontrol->value.integer.value[0];

	if (rotation < 0 || rotation > 3)
		return -EINVAL;
	if (tfa98xx_spk_rotation == rotation)
		return 0;

	tfa98xx_spk_rotation = rotation;
	tfa98xx_apply_spk_rotation();
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	nr_controls += 1; /* Speaker Rotation */
	...
	/* Speaker rotation control (0=0°, 1=90°, 2=180°, 3=270°) */
	tfa98xx_controls[mix_index].name = "TFA Speaker Rotation";
	tfa98xx_controls[mix_index].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	tfa98xx_controls[mix_index].info = tfa98xx_info_spk_rotation;
	tfa98xx_controls[mix_index].get = tfa98xx_get_spk_rotation;
	tfa98xx_controls[mix_index].put = tfa98xx_set_spk_rotation;
	mix_index++;
