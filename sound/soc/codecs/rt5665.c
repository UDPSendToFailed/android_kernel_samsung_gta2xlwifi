...
static bool rt5665_switch_registered;
#endif
...
static void rt5665_calibrate_handler(struct work_struct *work)
{
	...
	while (!rt5665->codec || !rt5665->codec->component.card ||
	       !rt5665->codec->component.card->instantiated) {
		pr_debug("%s\n", __func__);
		msleep(20);
	}
...
#ifdef CONFIG_SWITCH
	if (!switch_dev_register(&rt5665_headset_switch))
		rt5665_switch_registered = true;
#endif
