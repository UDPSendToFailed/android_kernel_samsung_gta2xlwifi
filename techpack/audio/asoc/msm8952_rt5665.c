...
static int msm8952_enable_dig_cdc_clk(struct snd_soc_card *card, int enable,
					bool dapm);
						    ...
						    int enable)
{
	...
	if (enable) {
		if (atomic_inc_return(&pdata->rt5665_mclk_ref) > 1)
			return;
		...
		if (ret < 0) {
			...
			atomic_dec(&pdata->rt5665_mclk_ref);
			return;
		}
		...
		if (atomic_dec_return(&pdata->rt5665_mclk_ref) < 0) {
			atomic_set(&pdata->rt5665_mclk_ref, 0);
			return;
		}
...
static int msm8952_enable_dig_cdc_clk(struct snd_soc_card *card,
...
static int msm_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	ret =  msm8952_enable_dig_cdc_clk(card, 1, true);
	if (ret < 0) {
	...
	if (ret < 0)
		pr_debug("%s: pri_i2s gpio set not defined, skipping\n",
				__func__);
...
static int msm_prim_auxpcm_startup(struct snd_pcm_substream *substream)
{
	...
	msm8952_enable_dig_cdc_clk(card, 1, true);
...
static void msm_prim_auxpcm_shutdown(struct snd_pcm_substream *substream)
{
	...
	if ((atomic_read(&auxpcm_mi2s_clk_ref) == 0) &&
		...
		msm8952_enable_dig_cdc_clk(card, 0, true);
	}
...
static int rt5665_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	snd_soc_dapm_ignore_suspend(snd_soc_codec_get_dapm(codec), "AIF2_1 Playback");
	snd_soc_dapm_ignore_suspend(snd_soc_codec_get_dapm(codec), "AIF2_1 Capture");
				...
				enum snd_soc_bias_level level)
{
	...
	if (!pdata->codec || dapm != snd_soc_codec_get_dapm(pdata->codec))
		return 0;
