...
static inline void sec_debug_save_badmode_info(int reason,
		const char *handler_str, unsigned int esr, const char *esr_str)
{
	_kern_ex_info_t *p_ex_info;
...
static inline char *sec_debug_get_reset_reason_str(unsigned int reason)
{
	return NULL;
}
...
static inline void sec_debug_save_fault_info(unsigned int esr, const char *str,
		unsigned long var1, unsigned long var2) {}
static inline struct debug_reset_header *get_debug_reset_header(void)
{
	return NULL;
}
