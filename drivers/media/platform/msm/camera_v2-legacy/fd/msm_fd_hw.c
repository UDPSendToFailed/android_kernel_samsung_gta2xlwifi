	...
	struct msm_fd_buffer *buffer)
{
	struct msm_fd_buf_handle *buf_handle =
		buffer->vb.vb2_buf.planes[0].mem_priv;
	...
	struct vb2_queue *vb2_q)
{
	...
	if (active_buffer) {
		...
		if (!time) {
			...
			vb2_buffer_done(&active_buffer->vb.vb2_buf, VB2_BUF_STATE_DONE);
