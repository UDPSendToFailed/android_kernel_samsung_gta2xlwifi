...
int msm_fd_buf_init(struct vb2_buffer *vb)
{
	...
	struct msm_fd_buffer *fd_buffer =
		container_of(vbuf, struct msm_fd_buffer, vb);
...
{
	struct msm_fd_mem_pool *pool = (struct msm_fd_mem_pool *)alloc_ctx;
...
static void msm_fd_wq_handler(struct work_struct *work)
{
	...
	ctx = vb2_get_drv_priv(active_buf->vb.vb2_buf.vb2_queue);
	...
	active_buf->vb.sequence = ctx->fh.sequence;
	...
	fd_event->buf_index = active_buf->vb.vb2_buf.index;
