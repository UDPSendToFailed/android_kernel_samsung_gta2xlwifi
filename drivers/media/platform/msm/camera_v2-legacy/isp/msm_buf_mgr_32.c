	...
	struct vb2_buffer *vb)
{
	...
	qbuf_buf->num_planes = vb->num_planes;
	for (i = 0; i < qbuf_buf->num_planes; i++) {
		qbuf_buf->planes[i].addr = vb->planes[i].m.userptr;
		qbuf_buf->planes[i].offset = vb->planes[i].data_offset;
	}
}
	...
	struct msm_isp_qbuf_buffer *qbuf_buf)
{
	...
	for (i = 0; i < qbuf_buf->num_planes; i++) {
		...
		mapped_info->handle =
		ion_import_dma_buf_fd(buf_mgr->client,
...
static int msm_isp_buf_prepare(struct msm_isp_buf_mgr *buf_mgr,
	struct msm_isp_qbuf_info *info, struct vb2_v4l2_buffer *vb2_buf)
{
	int rc = -1;
	unsigned long flags;
	struct msm_isp_bufq *bufq = NULL;
	...
	if (vb2_buf) {
		msm_isp_copy_planes_from_v4l2_buffer(&buf, &vb2_buf->vb2_buf);
	...
	uint32_t bufq_handle, struct msm_isp_buffer **buf_info)
{
	...
	struct msm_isp_buffer *temp_buf_info;
	struct msm_isp_bufq *bufq = NULL;
	struct vb2_v4l2_buffer *vb2_buf = NULL;
	struct buffer_cmd *buf_pending = NULL;
	...
	*buf_info = NULL;
	...
	case MSM_ISP_BUFFER_SRC_HAL:
		vb2_buf = buf_mgr->vb2_ops->get_buf(
			bufq->session_id, bufq->stream_id);
		if (vb2_buf) {
			if (vb2_buf->vb2_buf.index < bufq->num_bufs) {
				...
				list_for_each_entry(buf_pending,
					...
					mped_info_tmp1 =
						buf_pending->mapped_info;
					mped_info_tmp2 =
					&bufq->bufs[vb2_buf->vb2_buf.index]
						.mapped_info[0];
				...
				pr_err("%s: Incorrect buf index %d\n",
					__func__, vb2_buf->vb2_buf.index);
				rc = -EINVAL;
			}
		}
		break;
	...
	struct timeval *tv, uint32_t frame_id, uint32_t output_format)
{
	...
	if (state == MSM_ISP_BUFFER_STATE_DEQUEUED ||
		...
		spin_unlock_irqrestore(&bufq->bufq_lock, flags);
		if (MSM_ISP_BUFFER_SRC_HAL == BUF_SRC(bufq->stream_id)) {
			buf_info->vb2_buf->vb2_buf.timestamp =
				((u64)tv->tv_sec * 1000000 + tv->tv_usec) * 1000;
			buf_info->vb2_buf->sequence  = frame_id;
			buf_info->vb2_buf->timecode.type  = output_format;
