...
int sec_tclm_get_nvm_all(struct sec_tclm_data *data)
{
	...
	if (data->nvdata.cal_count == 0xFF ||
	    data->nvdata.cal_position >= CALPOSITION_MAX) {
	...
	input_info(true, &data->client->dev,
		"%s: cal_count:%d, pos:%d(%4s), hist_count:%d, lastp:%d\n",
...
void sec_tclm_position_history(struct sec_tclm_data *data)
{
	...
	for (index = 0; index < data->nvdata.cal_pos_hist_cnt; index++) {
		pr_cont("%c%d",
			data->tclm_string[data->nvdata.cal_pos_hist_queue[2 * now_lastp]].s_name,
			data->nvdata.cal_pos_hist_queue[2 * now_lastp + 1]);

		if (index < CAL_HISTORY_QUEUE_SHORT_DISPLAY) {
			data->cal_pos_hist_last3[2 * index] =
				data->tclm_string[
					data->nvdata.cal_pos_hist_queue[2 * now_lastp]].s_name;
			data->cal_pos_hist_last3[2 * index + 1] =
				data->nvdata.cal_pos_hist_queue[2 * now_lastp + 1];
		}
	...
	if (index < CAL_HISTORY_QUEUE_SHORT_DISPLAY)
		data->cal_pos_hist_last3[2 * index] = 0;
	else
...
void sec_tclm_root_of_cal(struct sec_tclm_data *data, int pos)
{
	...
	input_info(true, &data->client->dev, "%s: root - %d(%4s)\n",
		__func__, pos, data->tclm_string[pos].f_name);
}

static bool sec_tclm_check_condition_valid(struct sec_tclm_data *data)
{
	input_err(true, &data->client->dev,
		"%s tclm_level:%02X, last pos:%d(%4s), now pos:%d(%4s)\n",
		__func__, data->tclm_level, data->nvdata.cal_position,
		data->tclm_string[data->nvdata.cal_position].f_name,
		data->root_of_calibration,
		data->tclm_string[data->root_of_calibration].f_name);
...
void sec_tclm_reposition_history(struct sec_tclm_data *data)
{
	...
	data->nvdata.cal_pos_hist_queue[data->nvdata.cal_pos_hist_lastp * 2] =
		data->nvdata.cal_position;
	data->nvdata.cal_pos_hist_queue[data->nvdata.cal_pos_hist_lastp * 2 + 1] =
		data->nvdata.cal_count;
}
...
int sec_execute_tclm_package(struct sec_tclm_data *data, int factory_mode)
{
	...
	if (ret < 0) {
		input_err(true, &data->client->dev,
			"%s: sec_tclm_nvm_all_data i2c read fail", __func__);
		goto out;
	}

	input_err(true, &data->client->dev,
		"%s: tclm_level:%02X, last pos:%d(%4s), now pos:%d(%4s), factory:%d\n",
		__func__, data->tclm_level, data->nvdata.cal_position,
		data->tclm_string[data->nvdata.cal_position].f_name,
		data->root_of_calibration,
		data->tclm_string[data->root_of_calibration].f_name,
	...
	if (!factory_mode) {
		...
		ret = data->tclm_execute_force_calibration(data->client,
			TCLM_OFFSET_CAL_SEC);
		if (ret < 0) {
			input_err(true, &data->client->dev,
	...
	if (ret < 0) {
		input_err(true, &data->client->dev,
			"%s: SEC_TCLM_NVM_OFFSET_IC_FIRMWARE_VER i2c read fail",
			__func__);
		goto out;
	}
...
int sec_tclm_check_cal_case(struct sec_tclm_data *data)
{
	...
	if (data->nvdata.cal_count == 0xFF) {
		...
		if (ret < 0) {
			input_err(true, &data->client->dev,
				"%s: fail to read SEC_TCLM_NVM_ALL_DATA !\n", __func__);
			return ret;
		}
	...
	if (restore_cal) {
		...
		if (ret < 0) {
			input_err(true, &data->client->dev,
				"%s: fail sec_execute_tclm_package!\n", __func__);
			return ret;
		}
