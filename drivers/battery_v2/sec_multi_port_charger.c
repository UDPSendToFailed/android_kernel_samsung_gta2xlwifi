		...
		int * main_charging_current, int * sub_charging_current)
{
	...
	charger = power_supply_get_drvdata(psy_chg);
...
static int sec_multi_charger_probe(struct platform_device *pdev)
{
	...
	charger->psy_chg_desc.name           = "sec-multi-charger";
	charger->psy_chg_desc.type           = POWER_SUPPLY_TYPE_UNKNOWN;
	charger->psy_chg_desc.get_property   = sec_multi_chg_get_property;
	charger->psy_chg_desc.set_property   = sec_multi_chg_set_property;
	charger->psy_chg_desc.properties     = sec_multi_charger_props;
	charger->psy_chg_desc.num_properties = ARRAY_SIZE(sec_multi_charger_props);
