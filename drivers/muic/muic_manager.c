...
static void muic_manager_show_status(struct muic_interface_t *muic_if)
{
	...
	if (!ccic) {
		pr_info("%s: ccic not yet initialized\n", __func__);
		return;
	}
...
struct muic_interface_t *muic_manager_init(void *pdata, void *drv_data)
{
	...
	/* Initialize the mutex before registering the notifier, because
	 * ccic_notifier_register() replays buffered events synchronously
	 * and the handler (muic_manager_handle_notification) needs to
	 * acquire this mutex.  On 4.9, the USB-PD controller (s2mm005)
	 * probes before MUIC, so events are already queued.
	 */
