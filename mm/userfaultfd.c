...
retry:
	 ...
	 *
	 * Return -ENOENT (not -EINVAL) when the VMA is missing or
	 * the address falls outside the VMA bounds.  ART's CMC GC
	 * tolerates ENOENT from UFFDIO_ZEROPAGE when a concurrent
	 * thread has already unmapped the region, but treats EINVAL
	 * as a fatal error.  Upstream kernels (5.x+) return ENOENT
	 * here; match that behaviour.
	 */
	err = -ENOENT;
	...
	/*
	 * If the VMA was unregistered from userfaultfd (e.g. ART
	 * called UFFDIO_UNREGISTER after compaction), return -ENOENT
	 * so userspace treats this as a benign race rather than a
	 * fatal EINVAL.  Matches upstream find_dst_vma() behaviour.
	 */
	err = -ENOENT;
